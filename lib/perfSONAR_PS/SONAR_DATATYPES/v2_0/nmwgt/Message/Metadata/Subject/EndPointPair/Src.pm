                ...
                if(ref($query->{$table}{$entry}) eq 'ARRAY') {
                    foreach my $classes (@{$query->{$table}{$entry}}) {
                         if($classes && $classes eq 'perfSONAR_PS::SONAR_DATATYPES::v2_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src') {
                            ...
                            if    ($self->value && ( (  ( ($self->get_type eq 'hostname')  && $entry eq 'ip_name_src')) || (  ( ($self->get_type eq 'hostname')  && $entry eq 'ip_name') or  ( ($self->get_type eq 'ipv4')  && $entry eq 'ip_number')) )) {
                                $query->{$table}{$entry} =  $self->get_value;
                                $self->get_LOGGER->debug(" Got value for SQL query $table.$entry: " . $self->get_value);
                                last;  
                            }

                            elsif ($self->text && ( (  ( ($self->get_type eq 'hostname')  && $entry eq 'ip_name_src')) || (  ( ($self->get_type eq 'hostname')  && $entry eq 'ip_name') or  ( ($self->get_type eq 'ipv4')  && $entry eq 'ip_number')) )) {
