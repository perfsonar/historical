...
sub handleNonCompatEvent {
    ...
    if ($messageType eq "SetupDataRequest") {
        ...
        $self->handleStoreRequest($output, $metadataId, $ids[0], $responseType, $knowledge, $do_update, $d);
    }
...
=head2 handleStoreRequest ($self, $output, $metadataId, $id, $responseType, $knowledge, $do_update, $data)
...
sub handleStoreRequest {
    my ($self, $output, $metadataId, $id, $responseType, $knowledge, $do_update, $d) = @_;
    ...
    ($status, $res) = $self->{CLIENT}->updateLinkStatus($time, $d, $knowledge, $operState, $adminState, $do_update);
    if ($status != 0) {
