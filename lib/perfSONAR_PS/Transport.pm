...
sub acceptCall {
  ...
  (my $requestEndpoint = $http_request->uri) =~ s/^(\/)+//g;

  if($requestEndpoint ne $self->{LISTEN_ENDPOINT}) {
    $msg = "Received message with 'INVALID ENDPOINT'.";
    $logger->error($msg);     
    $request->setResponse(getResultCodeMessage("message.".genuid(), "", "", "response", "error.perfSONAR_PS.transport", $msg));  
  ...
  if ($request->parse($self->{NAMESPACES}, \$error_msg) != 0) {
    $request->setResponse(getResultCodeMessage("message.".genuid(), "", "", "ErrorResponse", "error.common.parse_error", $error_msg));
    return 0;
  }
  ...
  if($messageType eq "EchoRequest") { 
    ...
    foreach my $d ($request->getRequestDOM->getDocumentElement()->getElementsByTagNameNS($self->{NAMESPACE}, "data")) {      
      ...
      if(defined $m) {        
        ...
        my $eventType = extract(find($m, "./nmwg:eventType", 1));
        if($eventType eq "http://schemas.perfsonar.net/tools/admin/echo/2.0" or 
           $eventType eq "http://schemas.perfsonar.net/tools/admin/echo/ls/2.0" or
           $eventType eq "http://schemas.perfsonar.net/tools/admin/echo/ma/2.0" or
           $eventType =~ m/^echo.*/i) {
    ...
    $request->setResponse(getResultMessage("message.".genuid(), $messageId, "EchoResponse", $localContent));   
    return 0;
  }
  else {  
    $request->setRequestDOM(chainMetadata($request->getRequestDOM, $self->{NAMESPACE}));   
