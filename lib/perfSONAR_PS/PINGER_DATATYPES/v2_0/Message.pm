...
sub addResultData  {
    ...
    my $data  =  perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Data->new({ id => "data" . $self->dataID, metadataIdRef => "meta" . $params->{metadataID}, 
	                     datum =>  [perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwgr::Message::Data::Datum->new({text =>  $params->{message}})] });
    $self->addData($data);
    $self->add_dataID;
    return  $current_id;  
}
...
sub addResultResponse( ) {
    ...
    unless($params->{md} && blessed $params->{md}) {
       $params->{md}  = perfSONAR_PS::PINGER_DATATYPES::v2_0::nmwg::Message::Metadata->new();
