package  perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Description;
use strict;
use warnings;
...
=head1 NAME
 ...
 perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Description  - A base class, implements  'description'  element from the perfSONAR_PS RelaxNG schema
  
=head1 DESCRIPTION

   Object representation of the description element.
   Object fields are:
   
   The constructor accepts only single parameter, it could be a hashref to parameters hash  or DOM with  'description' element 
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Description->new($DOM_Obj);
...
use fields qw(nsmap idmap refidmap     text );

perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Description->mk_accessors(perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Description->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Description';
...
=head2   getDOM ($) 
       ...
       accept parent DOM
       return description object DOM, generated from object contents 
...
sub getDOM {
    ...
    my $parent = shift; 
    my $logger  = get_logger( $CLASSPATH ); 
    my $description = getElement({name =>   $LOCALNAME, parent => $parent , ns => [$self->nsmap->mapname( $LOCALNAME )],
                             attributes => [
...
=head2 merge
      ...
      merge with another description ( append + overwrite if exists )
...
sub merge {
    ...
    my $logger  = get_logger( $CLASSPATH );  
    unless($new_description && blessed $new_description && $new_description->can("getDOM")) {
        $logger->error(" Please supply defined object of description  ");
        return;
    } 
    ...
    if($self->can($member_name)) {
        ...
        $logger->error(" This field $member_name,  found in supplied  description  is not supported by description class");
        return;
        }
    }
    return $self;
...
=head2  asString()
   ...
   returns XML string  representation of the  description object
...
=head2 registerNamespaces ()
   ...
   will parse all subelements and register all namepspaces within the description namespace
...
sub fromDOM {
    ...
    $self->text($dom->textContent) if($dom->textContent);
