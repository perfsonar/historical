package  perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::HostName;
use strict;
use warnings;
...
=head1 NAME
 ...
 perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::HostName  - A base class, implements  'hostName'  element from the perfSONAR_PS RelaxNG schema
  
=head1 DESCRIPTION

   Object representation of the hostName element.
   Object fields are:
   
   The constructor accepts only single parameter, it could be a hashref to parameters hash  or DOM with  'hostName' element 
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::HostName->new($DOM_Obj);
...
perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::HostName->mk_accessors(perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::HostName->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::HostName';
...
=head2   getDOM ($) 
       ...
       accept parent DOM
       return hostName object DOM, generated from object contents 
...
sub getDOM {
    ...
    my $parent = shift; 
    my $logger  = get_logger( $CLASSPATH ); 
    my $hostName = getElement({name =>   $LOCALNAME, parent => $parent , ns => [$self->nsmap->mapname( $LOCALNAME )],
                             attributes => [
...
=head2 merge
      ...
      merge with another hostName ( append + overwrite if exists )
...
sub merge {
    ...
    my $logger  = get_logger( $CLASSPATH );  
    unless($new_hostName && blessed $new_hostName && $new_hostName->can("getDOM")) {
        $logger->error(" Please supply defined object of hostName  ");
        return;
    } 
    ...
    if($self->can($member_name)) {
        ...
        $logger->error(" This field $member_name,  found in supplied  hostName  is not supported by hostName class");
        return;
        }
    }
    return $self;
...
=head2  asString()
   ...
   returns XML string  representation of the  hostName object
...
=head2 registerNamespaces ()
   ...
   will parse all subelements and register all namepspaces within the hostName namespace
