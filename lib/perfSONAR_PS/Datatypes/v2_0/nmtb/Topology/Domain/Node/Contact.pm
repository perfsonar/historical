package  perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Contact;
use strict;
use warnings;
...
=head1 NAME
 ...
 perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Contact  - A base class, implements  'contact'  element from the perfSONAR_PS RelaxNG schema
  
=head1 DESCRIPTION

   Object representation of the contact element.
   Object fields are:
   ...
   The constructor accepts only single parameter, it could be a hashref to parameters hash  or DOM with  'contact' element 
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Contact->new($DOM_Obj);
...
use fields qw(nsmap idmap refidmap   email phoneNumber administrator institution );

perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Contact->mk_accessors(perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Contact->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Node::Contact';
...
=head2   getDOM ($) 
       ...
       accept parent DOM
       return contact object DOM, generated from object contents 
...
sub getDOM {
    ...
    my $parent = shift; 
    my $logger  = get_logger( $CLASSPATH ); 
    my $contact = getElement({name =>   $LOCALNAME, parent => $parent , ns => [$self->nsmap->mapname( $LOCALNAME )],
                             attributes => [
   ...
   foreach my $textnode (qw/email phoneNumber administrator institution /) {
       if($self->{$textnode}) { 
            my  $domtext  =  getElement({name =>   $textnode, parent => $contact , ns => [$self->nsmap->mapname($LOCALNAME)],
                                          text => $self->{$textnode},
                                 });
            $domtext?$contact->appendChild($domtext):$logger->error("Failed to append new text element $textnode  to  contact   ");
        } 
   } 
    return $contact;
}
...
=head2 merge
      ...
      merge with another contact ( append + overwrite if exists )
...
sub merge {
    my $self = shift;
    my $new_contact = shift;
    my $logger  = get_logger( $CLASSPATH );  
    unless($new_contact && blessed $new_contact && $new_contact->can("getDOM")) {
        $logger->error(" Please supply defined object of contact  ");
        return;
    } 
    ...
    if($self->can($member_name)) {
        ...
        $logger->error(" This field $member_name,  found in supplied  contact  is not supported by contact class");
        return;
        }
    }
    return $self;
...
=head2  asString()
   ...
   returns XML string  representation of the  contact object
...
=head2 registerNamespaces ()
   ...
   will parse all subelements and register all namepspaces within the contact namespace
