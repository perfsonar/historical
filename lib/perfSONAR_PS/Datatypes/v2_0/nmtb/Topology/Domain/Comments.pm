package  perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Comments;
use strict;
use warnings;
...
=head1 NAME
 ...
 perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Comments  - A base class, implements  'comments'  element from the perfSONAR_PS RelaxNG schema
  
=head1 DESCRIPTION

   Object representation of the comments element.
   Object fields are:
   
   The constructor accepts only single parameter, it could be a hashref to parameters hash  or DOM with  'comments' element 
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Comments->new($DOM_Obj);
...
perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Comments->mk_accessors(perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Comments->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::nmtb::Topology::Domain::Comments';
...
=head2   getDOM ($) 
       ...
       accept parent DOM
       return comments object DOM, generated from object contents 
...
sub getDOM {
    ...
    my $parent = shift; 
    my $logger  = get_logger( $CLASSPATH ); 
    my $comments = getElement({name =>   $LOCALNAME, parent => $parent , ns => [$self->nsmap->mapname( $LOCALNAME )],
                             attributes => [
...
=head2 merge
      ...
      merge with another comments ( append + overwrite if exists )
...
sub merge {
    ...
    my $logger  = get_logger( $CLASSPATH );  
    unless($new_comments && blessed $new_comments && $new_comments->can("getDOM")) {
        $logger->error(" Please supply defined object of comments  ");
        return;
    } 
    ...
    if($self->can($member_name)) {
        ...
        $logger->error(" This field $member_name,  found in supplied  comments  is not supported by comments class");
        return;
        }
    }
    return $self;
...
=head2  asString()
   ...
   returns XML string  representation of the  comments object
...
=head2 registerNamespaces ()
   ...
   will parse all subelements and register all namepspaces within the comments namespace
