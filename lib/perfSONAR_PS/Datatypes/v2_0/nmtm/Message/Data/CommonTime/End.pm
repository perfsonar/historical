package  perfSONAR_PS::Datatypes::v2_0::nmtm::Message::Data::CommonTime::End;
use strict;
use warnings;
...
perfSONAR_PS::Datatypes::v2_0::nmtm::Message::Data::CommonTime::End->mk_accessors(perfSONAR_PS::Datatypes::v2_0::nmtm::Message::Data::CommonTime::End->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::nmtm::Message::Data::CommonTime::End';
...
sub getDOM {
    ...
    my $parent = shift; 
    my $logger  = get_logger( $CLASSPATH ); 
    my $end = getElement({name =>   $LOCALNAME, parent => $parent , ns => [$self->nsmap->mapname( $LOCALNAME )],
                             attributes => [
...
sub merge {
    ...
    my $new_end = shift;
    my $logger  = get_logger( $CLASSPATH );  
    unless($new_end && blessed $new_end && $new_end->can("getDOM")) {
        $logger->error(" Please supply defined object of end  ");
        return;
    } 
    ...
    if($self->can($member_name)) {
        ...
        $logger->error(" This field $member_name,  found in supplied  end  is not supported by end class");
        return;
        }
    }
    return $self;
