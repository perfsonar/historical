...
sub getDOM {
    ...
    my $parameter = getElement({name =>   $LOCALNAME, parent => $parent , ns => \@nss,
                             attributes => [
...
sub  querySQL {
    ...
    my %defined_table = (  'time' => [  'cf',   'resolution',   'end',   'start',  ],   'metaData' => [  'protocol',   'transport',   'count',   'packetSize',   'ttl',   'packetInterval',  ],   'limit' => [  'setLimit',  ],  );
    ...
    $query->{time}{resolution}= [ 'perfSONAR_PS::Datatypes::v2_0::nmwg::Message::Metadata::Parameters::Parameter' ] if!(defined $query->{time}{resolution}) || ref($query->{time}{resolution});
    $query->{time}{cf}= [ 'perfSONAR_PS::Datatypes::v2_0::nmwg::Message::Metadata::Parameters::Parameter' ] if!(defined $query->{time}{cf}) || ref($query->{time}{cf});
