...
=head1 SYNOPSIS
	      ...
	      my $el =  perfSONAR_PS::Datatypes::v2_0::nmwg::Message->new(\%parameters);
...
use
  perfSONAR_PS::Datatypes::v2_0::pinger::Message::Metadata::Subject::Parameters;
...
perfSONAR_PS::Datatypes::v2_0::nmwg::Message->mk_accessors(
    perfSONAR_PS::Datatypes::v2_0::nmwg::Message->show_fields('Public') );
...
Readonly::Scalar our $CLASSPATH =>
  'perfSONAR_PS::Datatypes::v2_0::nmwg::Message';
...
sub new {
    ...
    if ($param) {
        ...
        $logger->debug(
            "Parsing parameters: " . ( join " : ", keys %{$param} ) );
...
sub getDOM {
    ...
    my $parent  = shift;
    my $logger  = get_logger($CLASSPATH);
    my $message = getElement(
        {
            name       => $LOCALNAME,
            parent     => $parent,
            ns         => [ $self->nsmap->mapname($LOCALNAME) ],
            attributes => [
    ...
    if ( $self->parameters && ref( $self->parameters ) eq 'ARRAY' ) {
        foreach my $subel ( @{ $self->parameters } ) {
            if ( blessed $subel && $subel->can("getDOM") ) {
                ...
                $subDOM
                  ? $message->appendChild($subDOM)
                  : $logger->error(
                    "Failed to append  parameters elements  with value: "
                      . $subDOM->toString );
            }
        }
    }
        ...
        foreach my $subel ( @{ $self->metadata } ) {
            if ( blessed $subel && $subel->can("getDOM") ) {
                ...
                $subDOM
                  ? $message->appendChild($subDOM)
                  : $logger->error(
                    "Failed to append  metadata elements  with value: "
                      . $subDOM->toString );
            }
        }
    }
        ...
        foreach my $subel ( @{ $self->data } ) {
            if ( blessed $subel && $subel->can("getDOM") ) {
                ...
                $subDOM
                  ? $message->appendChild($subDOM)
                  : $logger->error(
                    "Failed to append  data elements  with value: "
                      . $subDOM->toString );
            }
        }
    }
...
sub addParameters {
    ...
    $self->parameters && ref( $self->parameters ) eq 'ARRAY'
      ? push @{ $self->parameters }, $new
      : $self->parameters( [$new] );
...
sub removeParametersById {
    ...
    my $id     = shift;
    my $logger = get_logger($CLASSPATH);
    if (   ref( $self->parameters ) eq 'ARRAY'
        && $self->idmap->{parameters}
        && exists $self->idmap->{parameters}{$id} )
    {
        ...
        $logger->warn(
"Failed to remove  element because parameters not an array for non-existent id:$id"
        );
    }
    else {
...
sub getParametersByMetadataIdRef {
    ...
    my $logger = get_logger($CLASSPATH);
    if (   ref( $self->parameters ) eq 'ARRAY'
        && $self->refidmap->{parameters}
        && exists $self->refidmap->{parameters}{$id} )
    {
        my $parameters =
          $self->parameters->[ $self->refidmap->{parameters}{$id} ];
        return ( $parameters->can("metadataIdRef")
              && $parameters->metadataIdRef eq $id ) ? $parameters : undef;
    }
    elsif (
        $self->parameters
        && (
            ...
            || (   ref( $self->parameters ) ne 'ARRAY'
                && blessed $self->parameters
                && $self->parameters->can("metadataIdRef")
...
sub addMetadata {
    ...
    $self->metadata && ref( $self->metadata ) eq 'ARRAY'
      ? push @{ $self->metadata }, $new
      : $self->metadata( [$new] );
...
sub removeMetadataById {
    ...
    my $id     = shift;
    my $logger = get_logger($CLASSPATH);
    if (   ref( $self->metadata ) eq 'ARRAY'
        && $self->idmap->{metadata}
        && exists $self->idmap->{metadata}{$id} )
    {
        ...
        $logger->warn(
"Failed to remove  element because metadata not an array for non-existent id:$id"
        );
    }
    else {
        $logger->warn("Failed to remove element for non-existant id:$id");
...
sub getMetadataByMetadataIdRef {
    ...
    my $logger = get_logger($CLASSPATH);
    if (   ref( $self->metadata ) eq 'ARRAY'
        && $self->refidmap->{metadata}
        && exists $self->refidmap->{metadata}{$id} )
    {
        my $metadata = $self->metadata->[ $self->refidmap->{metadata}{$id} ];
        return ( $metadata->can("metadataIdRef")
              && $metadata->metadataIdRef eq $id ) ? $metadata : undef;
    }
    elsif (
        $self->metadata
        && (
            ...
            || (   ref( $self->metadata ) ne 'ARRAY'
                ...
                && $self->metadata->can("metadataIdRef")
...
sub addData {
    ...
    $self->data && ref( $self->data ) eq 'ARRAY'
      ? push @{ $self->data }, $new
      : $self->data( [$new] );
...
sub removeDataById {
        ...
        && exists $self->idmap->{data}{$id} )
    {
        ...
        $logger->warn(
"Failed to remove  element because data not an array for non-existent id:$id"
        );
    }
    else {
        $logger->warn("Failed to remove element for non-existant id:$id");
...
sub getDataByMetadataIdRef {
    ...
    if (   ref( $self->data ) eq 'ARRAY'
        && $self->refidmap->{data}
        && exists $self->refidmap->{data}{$id} )
    {
        my $data = $self->data->[ $self->refidmap->{data}{$id} ];
        return ( $data->can("metadataIdRef") && $data->metadataIdRef eq $id )
          ? $data
          : undef;
    }
    elsif (
...
sub querySQL {
    ...
    foreach my $subname (qw/parameters metadata data/) {
        if (
            $self->{$subname}
            && ( ref( $self->{$subname} ) eq 'ARRAY'
                || blessed $self->{$subname} )
          )
        {
            my @array =
              ref( $self->{$subname} ) eq 'ARRAY'
            ...
            foreach my $el (@array) {
                if ( blessed $el && $el->can("querySQL") ) {
                    ...
                    $logger->error(
" Failed for message Unblessed member or querySQL is not implemented by subclass $subname"
                    );
                }
            }
        }
    ...
    foreach my $member_name ( $new_message->show_fields ) {
        ...
        if ( $self->can($member_name) ) {
            ...
            if ( $current_member && $new_member ) {
                ...
                if (
                       blessed $current_member
                    && blessed $new_member
                    && $current_member->can("merge")
                    && ( $current_member->nsmap->mapname($member_name) eq
                        $new_member->nsmap->mapname($member_name) )
                  )
                {
                    $current_member->merge($new_member);
                    $self->{$member_name} = $current_member;
                    $logger->debug( "  Merged $member_name , got"
                          . $current_member->asString );
            ...
            $logger->error(
" This field $member_name,  found in supplied  message  is not supported by message class"
            );
            return;
        }
    }
    ...
    foreach my $childnode ( $dom->childNodes ) {
            ...
            && $self->can($tagname) )
        {
            ...
            if ( $EVAL_ERROR || !( $element && blessed $element) ) {
                $logger->error( " Failed to load and add  Parameters : "
                      . $dom->toString
                      . " error: "
                      . $EVAL_ERROR );
                return;
            ...
            && $self->can($tagname) )
        {
            ...
            eval {
                $element =
                  perfSONAR_PS::Datatypes::v2_0::nmwg::Message::Metadata->new(
                    $childnode);
            };
            if ( $EVAL_ERROR || !( $element && blessed $element) ) {
                $logger->error( " Failed to load and add  Metadata : "
                      . $dom->toString
                      . " error: "
                      . $EVAL_ERROR );
                return;
        ...
        elsif ( $tagname eq 'data' && $nsid eq 'nmwg' && $self->can($tagname) )
        {
            ...
            eval {
                $element =
                  perfSONAR_PS::Datatypes::v2_0::nmwg::Message::Data->new(
                    $childnode);
            };
            if ( $EVAL_ERROR || !( $element && blessed $element) ) {
                $logger->error( " Failed to load and add  Data : "
                      . $dom->toString
                      . " error: "
                      . $EVAL_ERROR );
                return;
