package  perfSONAR_PS::Datatypes::v2_0::nmwg::Topology::Domain::Node::Parameters;
use strict;
use warnings;
...
=head1 NAME
 ...
 perfSONAR_PS::Datatypes::v2_0::nmwg::Topology::Domain::Node::Parameters  - A base class, implements  'parameters'  element from the perfSONAR_PS RelaxNG schema
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::Datatypes::v2_0::nmwg::Topology::Domain::Node::Parameters->new($DOM_Obj);
...
perfSONAR_PS::Datatypes::v2_0::nmwg::Topology::Domain::Node::Parameters->mk_accessors(perfSONAR_PS::Datatypes::v2_0::nmwg::Topology::Domain::Node::Parameters->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::nmwg::Topology::Domain::Node::Parameters';
...
sub getDOM {
    ...
    my $parameters = getElement({name =>   $LOCALNAME, parent => $parent , ns => \@nss,
                             attributes => [
...
sub removeParameterById {
    ...
    if(ref($self->parameter) eq 'ARRAY' && $self->idmap->{parameter} &&  exists $self->idmap->{parameter}{$id}) { 
        undef $self->parameter->[$self->idmap->{parameter}{$id}]; 
...
sub fromDOM {
    ...
    foreach my $childnode ($dom->childNodes) { 
        ...
        if ($tagname eq  'parameter' && $nsid eq 'nmwg' && $self->can($tagname)) { 
           my $element = undef;
           eval {
               $element = perfSONAR_PS::Datatypes::v2_0::nmwg::Topology::Domain::Node::Parameters::Parameter->new($childnode) 
           };
