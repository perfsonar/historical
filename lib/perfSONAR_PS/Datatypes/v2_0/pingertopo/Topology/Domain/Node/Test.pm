package  perfSONAR_PS::Datatypes::v2_0::pingertopo::Topology::Domain::Node::Test;
use strict;
use warnings;
...
=head1 NAME
 ...
 perfSONAR_PS::Datatypes::v2_0::pingertopo::Topology::Domain::Node::Test  - A base class, implements  'test'  element from the perfSONAR_PS RelaxNG schema
  
=head1 DESCRIPTION

   Object representation of the test element.
   Object fields are:
    Scalar:     id, 
    Object reference:   packetSize => type ,
    Object reference:   count => type ,
    Object reference:   interval => type ,
    Object reference:   ttl => type ,
    Object reference:   period => type ,
    Object reference:   offset => type ,
   
   The constructor accepts only single parameter, it could be a hashref to parameters hash  or DOM with  'test' element 
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::Datatypes::v2_0::pingertopo::Topology::Domain::Node::Test->new($DOM_Obj);
...
use fields qw(nsmap idmap refidmap id  packetSize count interval ttl period offset );

perfSONAR_PS::Datatypes::v2_0::pingertopo::Topology::Domain::Node::Test->mk_accessors(perfSONAR_PS::Datatypes::v2_0::pingertopo::Topology::Domain::Node::Test->show_fields('Public'));
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::Datatypes::v2_0::pingertopo::Topology::Domain::Node::Test';
Readonly::Scalar our $LOCALNAME => 'test';
...
=head2   getDOM ($) 
       ...
       accept parent DOM
       return test object DOM, generated from object contents 
...
sub getDOM {
    ...
    my $parent = shift; 
    my $logger  = get_logger( $CLASSPATH ); 
    my $test = getElement({name =>   $LOCALNAME, parent => $parent , ns => [$self->nsmap->mapname( $LOCALNAME )],
                             attributes => [

                                               ['id' =>  $self->id],
                                           ],
                         }); 
   foreach my $textnode (qw/packetSize count interval ttl period offset /) {
       if($self->{$textnode}) { 
            my  $domtext  =  getElement({name =>   $textnode, parent => $test , ns => [$self->nsmap->mapname($LOCALNAME)],
                                          text => $self->{$textnode},
                                 });
            $domtext?$test->appendChild($domtext):$logger->error("Failed to append new text element $textnode  to  test   ");
        } 
   } 
    return $test;
}
...
=head2 merge
      ...
      merge with another test ( append + overwrite if exists )
...
sub merge {
    ...
    my $new_test = shift;
    my $logger  = get_logger( $CLASSPATH );  
    unless($new_test && blessed $new_test && $new_test->can("getDOM")) {
        $logger->error(" Please supply defined object of test  ");
        return;
    } 
    ...
    if($self->can($member_name)) {
        ...
        $logger->error(" This field $member_name,  found in supplied  test  is not supported by test class");
        return;
        }
    }
    return $self;
...
=head2  asString()
   ...
   returns XML string  representation of the  test object
...
=head2 registerNamespaces ()
   ...
   will parse all subelements and register all namepspaces within the test namespace
