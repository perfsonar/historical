...
sub owptstampdnum {
    my ( $sec, $min, $hour, $mday, $mon, $year, $wday, $yday ) = OWP::Utils::owpgmtime( shift );
...
sub owplocalstring {
    ...
    return strftime "%a %b %e %H:%M:%S %Z", localtime( $unixsecs );
}
...
L<Exporter>, L<Math::BigInt>, L<Math::BigFloat>, L<POSIX>
