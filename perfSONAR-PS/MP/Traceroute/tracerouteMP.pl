...
use perfSONAR_PS::MA::Traceroute;
...
my %ns = (
  ...
  trace => "http://ggf.org/ns/nmwg/tools/traceroute/2.0/",
  select => "http://ggf.org/ns/nmwg/ops/select/2.0/"
);

		# Read in configuration information
my %conf = ();
readConfiguration("./tracerouteMP.conf", \%conf);
...
sub measurementArchive {
  ...
  my $ma = new perfSONAR_PS::MA::Traceroute(\%conf, \%ns);
  $ma->init;  
  while(1) {
...
tracerouteMP.pl - An Traceroute based collection agent (MeasurementPoint) with 
MA (MeasurementArchive) capabilities.

=head1 DESCRIPTION

This script creates an MP and MA for an Traceroute based collector.  The service is 
also capable of registering with an LS instance.  

=head1 SYNOPSIS

./tracerouteMP.pl [--verbose | --help]
...
perfSONAR_PS::MP::Traceroute
perfSONAR_PS::MA::Traceroute
