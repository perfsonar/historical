...
%conf = ();
%result = ();
readConfiguration("./t/testfiles/blank", \%conf);
is(Compare(\%conf, \%result), 1, "Common::readConfiguration - Empty file");
 
%conf = ();
%result = ("t1" => "r1", "t2" => "r2");
readConfiguration("./t/testfiles/sampleconf", \%conf);
is(Compare(\%conf, \%result), 1, "Common::readConfiguration - Sample configuration file");
