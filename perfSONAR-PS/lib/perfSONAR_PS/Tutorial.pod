...
called functions as they exist in the skeletonMP module (which is now refered to
...
We are quick to realize that building the command string is imporant
...
ger our destination host.  Each of the auxiliary functions is defined in the
...
be examining some functions in the skeletonMA module (which is now refered 
...
for the tutorial.  If you require some extra data to be included into the base MP, 
this function I<MAY> be modified to suite your needs, although it is not recomended
as all of the MPs will use this as their starting point.  It is possible to redefine
