...
package perfSONAR_PS::MA::Traceroute;
...
sub receive {
  my($self) = @_;
  my $logger = get_logger("perfSONAR_PS::MA::Traceroute");
...
sub retrieveSQL {
  ...
  if($#{$result} == -1) {
    ...
    for(my $a = 0; $a <= $#{$result}; $a++) {    
      $responseString = $responseString . "    <traceroute:datum";
...
perfSONAR_PS::MA::Traceroute - A module that provides methods for the Traceroute MA.  
...
=head1 SYNOPSIS
    ...
    my $ma = perfSONAR_PS::MA::Traceroute->new(\%conf, \%ns);

    # or
    # $ma = perfSONAR_PS::MA::Traceroute->new;
    # $ma->setConf(\%conf);
    # $ma->setNamespaces(\%ns);      
