...
sub new {
  ...
  my %hash = ();
  $hash{"FILENAME"} = "perfSONAR_PS::MP::Owamp::Agent";
  $hash{"FUNCTION"} = "\"new\"";
...
sub setLog {
  my ($self, $log) = @_;  
  $self->{FILENAME} = "perfSONAR_PS::MP::Owamp::Agent";
...
# ================ Main Package perfSONAR_PS::MP::Owamp ================
...
sub new {
  ...
  my %hash = ();
  $hash{"FILENAME"} = "perfSONAR_PS::MP::Owamp";
  $hash{"FUNCTION"} = "\"new\"";
...
sub setConf {
  my ($self, $conf) = @_;  
  $self->{FILENAME} = "perfSONAR_PS::MP::Owamp";  
...
sub parseMetadata {
  ...
  if($self->{CONF}->{"METADATA_DB_TYPE"} eq "xmldb") {  
    ...
    print "#resultsStringMD=$#resultsStringMD\n";
    ...
    if($#resultsStringD != -1) {    
      for(my $x = 0; $x <= $#resultsStringD; $x++) { 	
        ...
        print "Metadata2:\t" , Dumper($self->{METADATA}) , "\n";
      }
    }
    else {
      my $msg = $self->{FILENAME} .":\tXMLDB returned 0 results for query '". $query ."' in function " . $self->{FUNCTION};
  ...
  foreach my $m (keys %{$self->{METADATA}}) {
    if($self->{METADATAMARKS}->{$m}) {
      ...
      if(!defined $host or $host eq "") {
        ...
        $self->{PING}->{$m} = new perfSONAR_PS::MP::Owamp::Agent(
	  $self->{CONF}->{"LOGFILE"},
          $commandString
	);
      }
...
perfSONAR_PS::MP::Owamp - A module starting point for MP functions...
...
a specialized structure for use only in this module.  The functions include:
  ...
  setLog($log)
    ...
    (Re-)Sets the log file for the Owamp object.


A brief description using the API:
   
    my $agent = new perfSONAR_PS::MP::Owamp::Agent("./error.log");
