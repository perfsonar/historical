...
perfSONAR_PS::MP::Agent::CommandLine - A module that will run a command and
return it's output.
...
=head1 SYNOPSIS
  ...
  my $agent = perfSONAR_PS::MP::Agent::CommandLine( $command, $options );
  $agent->init();
...
our $logger = Log::Log4perl::get_logger( 'perfSONAR_PS::MP::Agent::CommandLine' );
...
sub collectMeasurements
{
  ...
  if( defined $self->commandString() ) {   
    ...
    $logger->info( "Running '" . $self->commandString() . "'... ");
    my @results = <CMD>;
    close(CMD);
