...
sub parseMetadata {
  my($self) = @_;
  my $logger = get_logger("perfSONAR_PS::MP::Bwctl");
...
sub prepareCollectors {
  ...
  foreach my $m ($self->{STORE}->getElementsByTagNameNS($self->{NAMESPACES}->{"nmwg"}, "metadata")) {
    if($self->{METADATAMARKS}->{$m->getAttribute("id")}) {
      ...
      my $bwctlPrefix = lookup($self, "http://ggf.org/ns/nmwg/tools/bwctl/2.0/", "bwctl");    
      my $host = extract($m->find(".//".$topoPrefix.":dst")->get_node(1)); 
      my $count = extract($m->find(".//".$bwctlPrefix.":parameters/nmwg:parameter[\@name=\"count\"]")->get_node(1));
      ...
      if(!defined $host or $host eq "") {
        ...
        $self->{AGENT}->{$m->getAttribute("id")} = new perfSONAR_PS::MP::Bwctl::Agent(
          $commandString
	      );
      }     
    }    
      ...
      foreach my $r (sort keys %{$results}) {
        $logger->debug("Inserting \"".$d->getAttribute("metadataIdRef").
          "\", \"".$results->{$r}->{"timeValue"}."\", \"".$results->{$r}->{"time"}.
	        "\", \"bwctl\", \""."numBytes=".$results->{$r}->{"bytes"}.",ttl=".
...
# ================ Internal Package perfSONAR_PS::MP::Bwctl::Agent ================
...
sub setCommand {
  my ($self, $cmd) = @_;  
  my $logger = get_logger("perfSONAR_PS::MP::Bwctl::Agent");
  if(defined $cmd and $cmd ne "") {
    $self->{CMD} = $cmd;
  }
  else {
...
perfSONAR_PS::MP::Bwctl - A module that performs the tasks of an MP designed for the 
bwctl measurement.  

=head1 DESCRIPTION

The purpose of this module is to create simple objects that contain all necessary information
to make bwctl measurements to various hosts.  The objects can then be re-used with minimal 
effort.
...
=head1 SYNOPSIS
    ...
    $conf{"METADATA_DB_NAME"} = "/home/jason/perfSONAR-PS/MP/Bwctl/xmldb";
    $conf{"METADATA_DB_FILE"} = "bwctlstore.dbxml";
    $conf{"PING"} = "/bin/bwctl";
...
rather as a specialized structure for use only in this module.  The functions include:
  ...
  setCommand($cmd)
    ...
    (Re-)Sets the command for the bwctl agent object. 
...
A brief description using the API:
    ...
    my $agent = new perfSONAR_PS::MP::Bwctl::Agent("/bin/bwctl -c 1 localhost");

    # or also:
    # 
    # my $agent = new perfSONAR_PS::MP::Bwctl::Agent;
    # $agent->setCommand("/bin/bwctl -c 1 localhost");
...
Prepares the 'perfSONAR_PS::MP::Bwctl::Agent' objects for each of the metadata values in
the metadata object.

=head2 collectMeasurements()

Cycles through each of the 'perfSONAR_PS::MP::Bwctl::Agent' objects and gathers the 
necessary values.  
...
$Id: Bwctl.pm 216 2007-06-04 15:50:02Z zurawski $
