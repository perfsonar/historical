...
%define init_script_1 skeleton1
# %define init_script_2 skeleton2
...
make PREFIX=%{buildroot}/%{install_base} rpminstall

mkdir -p %{buildroot}/etc/init.d

awk "{gsub(/^PREFIX=.*/,\"PREFIX=%{install_base}\"); print}" scripts/%{init_script_1} > scripts/%{init_script_1}.new
install -m 755 scripts/%{init_script_1}.new %{buildroot}/etc/init.d/%{init_script_1}

# XXX Uncomment the following lines if you have multiple init scripts
#awk "{gsub(/^PREFIX=.*/,\"PREFIX=%{install_base}\"); print}" scripts/%{init_script_2} > scripts/%{init_script_2}.new
#install -m 755 scripts/%{init_script_2}.new %{buildroot}/etc/init.d/%{init_script_2}
