...
my %defaults = (
    ...
    rrd_create_parameters =>
        'RRA:%consolidation%:0.5:1:241920 RRA:%consolidation%:0.5:2:120960 RRA:%consolidation%:0.5:6:40320 RRA:%consolidation%:0.5:12:20160 RRA:%consolidation%:0.5:24:10080 RRA:%consolidation%:0.5:36:6720 RRA:%consolidation%:0.5:48:5040 RRA:%consolidation%:0.5:60:4032 RRA:%consolidation%:0.5:120:2016',
    rrd_step => 30,
);
...
sub add_port_metadata {
    ...
    ( $status, $res )
        = $self->__lookup_port_metadata(
        { urn => $parameters->{urn}, data_type => $parameters->{data_type}, host_name => $parameters->{host_name}, port_name => $parameters->{port_name}, capacity => $parameters->{capacity}, direction => $parameters->{direction}, description => $parameters->{description} } );
    if ( $status != 0 ) {
...
sub lookup_port_metadata {
    ...
    ( $status, $res )
        = $self->__lookup_port_metadata( { id => $parameters->{id}, urn => $parameters->{urn}, host_name => $parameters->{host_name}, port_name => $parameters->{port_name}, direction => $parameters->{direction}, capacity => $parameters->{capacity}, description => $parameters->{description} } );
