...
bwcollector.pl - program that collects owamp data

=head1 SYNOPSIS

bwcollector.pl [B<-a> add_this_file][B<-o>][B<-c> confdir/][B<-Z>][B<-k>][B<-h>][B<-p>]
...
in an "archive" directory. bwcollector.pl is also responsible for validating
the throughput session data. It needs to determine if lost packets are due to one
...
# cache per/test values
my %testid_ca;

my %id2node;    # key is nodeid
my %node2id;    # key is node

# globals used by "both" update and handle_req processes
my (%vstart);    # key is testname (TODO: max time seen?)
...
while (1) {
    ...
    if ( !defined($paddr) ) {
        ...
        if ($reset) {
            ...
            warn "Restarting...\n";
            die "Unable to exec!"
                if ( sigprocmask( SIG_SETMASK, $old_mask ) != 0 );
...
sub read_req {
    ...
    return if !defined $_;
    die "Invalid request!: $_" if ( !( ($vers) = /BW\s+(\d+)/ ) );
    $md5->add($_);
...
sub get_prev_tprefix {
    ...
    return (sprintf('%4.4d%2.2d', $year+1900, $mon+1),$year,$mon);
}
...
sub save_tspec {
    ...
    $sth->execute($digest,
            ...
            $req->{'BWWINDOWSIZE'},
            $req->{'BWTOS'},
            $req->{'BWPARALLELSTREAMS'} || 1,
            ($req->{'BWUDP'})? 1: 0,
            $req->{'BWUDPBANDWIDTHLIMIT'}) ||
...
sub save_node {
    ...
    $sth->execute($digest,$args{'NODE'},$args{'LONGNAME'},$args{'ADDR'},
            $first,$last) || ldie "Exec: Insert ${lname}_NODES";
...
sub add_session {
    ...
    my $testname = $reqh->{'MEASUREMENTSET'} . "_" .
            $reqh->{'RECVNODE'} . "_" . $reqh->{'SENDNODE'};
    ...
    WLOOP:
        while ($rlen) {
            ...
            if ( !defined $written ) {
                ...
                ldie "Write error to file $req{'FNAME'}.i: $!";
            }
            if ( $written < 1 ) {
                ldie "0 length write to $req{'FNAME'}.i: $!\n";
            }
            $rlen -= $written;
            $offset += $written;
        }
...
#        'ARCHIVE'  => $archive,
    ...
    add_session(%add_args) || ldie "Unable to add $req{'FNAME'}.i";
...
REQ_LOOP:
    while (1) {
        ...
        #
        last if ($die);
        eval { sigprocmask( SIG_SETMASK, $old_mask ); };
        last if ($die);
        die "\$@ = $@" if ($@);
        ...
        eval { sigprocmask( SIG_BLOCK, $block_mask ); };
        last if ($die);
        die "\$@ = $@" if ($@);
        undef %response;
        %response = do_req( $fh, $md5, $dbh, %req ) if (%req);
        next if ( %response
