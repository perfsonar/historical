...
sub init
{
    ...
    $self->{'m_plotAreaBgColor'} = 0xfffff0;
...
sub addVolBars2
{
    ...
    my $upLayer = $self->addBarIndicator2($c, new ArrayMath($self->{'m_volData'})->selectGTZ(
        $closeChange)->result(), $upColor, "");
    my $dnLayer = $self->addBarIndicator2($c, new ArrayMath($self->{'m_volData'})->selectLTZ(
        $closeChange)->result(), $downColor, "");
    my $flatLayer = $self->addBarIndicator2($c, new ArrayMath($self->{'m_volData'})->selectEQZ(
        $closeChange)->result(), $flatColor, "");

    my $maxVol = new ArrayMath($self->{'m_volData'})->max();
    ...
    my $volFormat = "";
    if ($maxVol < 10) {
        $volFormat = sprintf("{value|1}%s", $self->{'m_volUnit'});
    } else {
        $volFormat = sprintf("{value}%s", $self->{'m_volUnit'});
    }
    ...
    my $label = "Vol";
    if ($i >= 0) {
        $label = sprintf("Vol: %s%s", $self->formatValue($self->{'m_volData'}->[$i],
            $self->{'m_generalFormat'}), $self->{'m_volUnit'});
    }

    my $selectedLayer = undef;
    ...
    $selectedLayer->getDataSet(0)->setDataName($label);
...
sub addIndicator
{
    ...
    if (defined($self->{'m_currentChart'})) {
        ...
        $self->{'m_currentChart'}->xAxis()->setColors($perlchartdir::LineColor,
    ...
    $ret->setPlotArea($self->{'m_leftMargin'}, $self->{'m_topMargin'}, $self->{'m_totalWidth'} -
        ...
        )->setGridColor($self->{'m_majorHGridColor'}, $self->{'m_majorVGridColor'},
        $self->{'m_minorHGridColor'}, $self->{'m_minorVGridColor'});
    ...
    $box->setSize($self->{'m_totalWidth'} - $self->{'m_leftMargin'} - $self->{'m_rightMargin'}, 0);
...
sub addChaikinOscillator
{
    ...
    my $expAvg10 = new ArrayMath($accdist)->expAvg(2.0 / (11))->result();
    return $self->addLineIndicator($height, new ArrayMath($accdist)->expAvg(2.0 / (4))->sub(
...
{
    my ($self, $height, $color) = @_;
    my $firstCloseIndex = $self->{'m_extraPoints'};
    while ($firstCloseIndex < scalar(@{$self->{'m_closeData'}})) {
        if (($self->{'m_closeData'}->[$firstCloseIndex] != $perlchartdir::NoValue) && (
            $self->{'m_closeData'}->[$firstCloseIndex] != 0)) {
            last;
        }
        $firstCloseIndex = $firstCloseIndex + 1;
    }

    if ($firstCloseIndex < scalar(@{$self->{'m_closeData'}})) {
        return $self->addLineIndicator($height, new ArrayMath($self->{'m_closeData'})->div(
            $self->{'m_closeData'}->[$firstCloseIndex])->sub(1)->mul(100)->result(), $color,
            "Performance");
    } else {
        #chart is empty !!!
...
sub computeRSI
{
    ...
    my $absChange = new ArrayMath($self->{'m_closeData'})->delta()->abs()->movAvg($period)->result()
        ;
    return new ArrayMath($self->{'m_closeData'})->delta()->selectGTZ()->movAvg($period)->financeDiv(
        $absChange, 0.5)->mul(100)->result();
}
...
sub addTRIX
{
    ...
    return $self->addLineIndicator($height, new ArrayMath($self->{'m_closeData'})->expAvg($f
        )->expAvg($f)->expAvg($f)->rate()->result(), $color, $label);
}
