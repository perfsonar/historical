...
# Data for the chart
my $data0 = [1700, 3900, 2900, 3800, 4100, 4600, 2900, 4100, 4400, 5700, 5900, 5200,
    3700, 3400, 5100, 5600, 5600, 6000, 7000, 7600, 6300, 6700, 7500, 6400, 8800];
my $data1 = [500, 550, 670, 990, 820, 730, 800, 720, 730, 790, 860, 800, 840, 680,
    740, 890, 680, 790, 730, 770, 840, 820, 800, 840, 670];
my $data2 = [46, 68, 35, 33, 38, 20, 12, 18, 15, 23, 30, 24, 28, 15, 21, 26, 46, 42,
    38, 25, 23, 32, 24, 20, 25];
my $data3 = [0.84, 0.82, 0.82, 0.38, 0.25, 0.52, 0.54, 0.52, 0.38, 0.51, 0.46, 0.29,
...
my $legendBox = $c->addLegend(300, 70, 0, "arialbd.ttf", 8);
...
$c->yAxis()->setTitle("Power\n(Watt)")->setAlignment($perlchartdir::TopLeft2);
...
$c->yAxis2()->setTitle("Load\n(Mbps)")->setAlignment($perlchartdir::TopRight2);
...
my $leftAxis = $c->addAxis($perlchartdir::Left, 50);
# Add a title on top of the third y axis.
$leftAxis->setTitle("Temp\n(C)")->setAlignment($perlchartdir::TopLeft2);
...
my $rightAxis = $c->addAxis($perlchartdir::Right, 50);
# Add a title on top of the fourth y axis.
$rightAxis->setTitle("Error\n(%)")->setAlignment($perlchartdir::TopRight2);
...
# width of 2 pixels
my $layer0 = $c->addLineLayer($data0, 0xcc0000, "Power");
...
my $layer1 = $c->addLineLayer($data1, 0x008000, "Load");
...
my $layer2 = $c->addLineLayer($data2, 0x0000cc, "Temperature");
...
my $layer3 = $c->addLineLayer($data3, 0x880088, "Error Rate");
