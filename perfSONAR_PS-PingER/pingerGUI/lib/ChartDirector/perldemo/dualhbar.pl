...
my $male = [215, 238, 225, 236, 235, 260, 286, 340, 363, 305, 259, 164, 135, 127,
    102, 68, 66];

# The female population (in thousands)
my $female = [194, 203, 201, 220, 228, 271, 339, 401, 384, 304, 236, 137, 116, 122,
    112, 85, 110];
...
my $femaleLayer = $c->addBarLayer($female, 0xf0c0c0);

# Swap the axis so that the bars are drawn horizontally
$c->swapXY(1);

# Set the bar to touch each others
$femaleLayer->setBarGap($perlchartdir::TouchBar);
...
$c->addLineLayer($male, $perlchartdir::Transparent);
...
my $c2 = new XYChart(280, 300, $perlchartdir::Transparent);
...
my $maleLayer = $c2->addBarLayer($male, 0xaaaaff);
...
# Set the bar to touch each others
$maleLayer->setBarGap($perlchartdir::TouchBar);
...
# female data in both left and right charts, because we want auto-scaling to produce
# the same scale for both chart.
$c2->addLineLayer($female, $perlchartdir::Transparent);
...
$m->addTitle2($perlchartdir::Bottom, "Population (in thousands)", "arialbi.ttf", 10);
