...
landmarks.xml file. It can also update existing XML landmarks file by adding new
nodes from the CSV file. The CSV filename is supplied through the
--file=<filename> mandatory option.  The format of CSV file is:
    <domain>,<node>,<hostname>,<ip>,<description>,<packetsize>,<count>,<packetInterval>,<ttl>,<measurementPeriod>,<measuermentOffset>,<project>

First 4 parameters are required but if one of hostname or ip is missing then it
will atempt to resolve the rest.  Other parameters are optional and will be set
to default values:
    <description=''>,<packetsize=1000>,<count=10>,<packetInterval=1>,<ttl=255>,<measurementPeriod=60>,<measurementOffset=0>,<project=LHCOPN>
     
The default values could be set to different ones by using command line optional
options:
...
Other optional parameters are:
  ...
  --out=<filename> - sets the resulting filename, if skipped then default
                     filename will be created by substituting .csv extention
                     with .xml and keeping the same base filename.
  --update=<filename>  -  use this XML landmarks file to update, if --out option
                          is set then new file will be created if not then this
                          file will be overwritten
      
Note: if you updating an existing file then for any matching domain:node id all
parameters will be overwritten from the CSV file or reset to defaults if missing 
