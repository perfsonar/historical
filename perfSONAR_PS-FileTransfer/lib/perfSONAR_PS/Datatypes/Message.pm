...
It extends: 
    ...
    use perfSONAR_PS::FT_DATATYPES::v2_0::nmwg::Message; 
...
use base qw(perfSONAR_PS::FT_DATATYPES::v1_0::nmwg::Message);
...
sub new {
    ...
    my $param  = shift;
    my $nextID = shift
      ;    # This id field has been introduce, in order to counter the error of,
           # multiple Data object being returned with the same id.
           # The script making the object, is supposed to increment this id,
           # inorder to keep the objects unique. If the field is not set,
    ...
    #$self->get_LOGGER->info("sending to SUPER:".$param);
    $self = $self->SUPER::new($param);    # init base fields

    #$self->get_LOGGER->info("back from SUPER");
    $self->eventTypes( perfSONAR_PS::Datatypes::EventTypes->new() );
    if ( defined($nextID) && $nextID =~ m/\d+/i ) {
    ...
    #$self->get_LOGGER->info("New Message Call ended");
    return $self;
}
...
sub getDataByMetadataIdRef {
    ...
    #$self->get_LOGGER->info("IdRefData = ".Dumper $self->get_data);
    if ( ref( $self->get_data ) eq 'ARRAY' ) {
        foreach my $data ( @{ $self->get_data } ) {

            #$self->get_LOGGER->info($data->get_metadataIdRef."--".$idref);
    ...
    $self->get_LOGGER->warn(
        "Requested element for non-existent metadataIdRef: $idref");
    return;
}
...
"Parameters missed:  addResultData Usage:: addResultData(\$params) where \$params is hashref"
        );
        return;
    }
...
sub addResultResponse {
    ...
    unless ( $params && ref($params) eq 'HASH' && $params->{eventType} ) {
        $self->get_LOGGER->error(
"Parameters missed:  addResultResponse Usage:: addResultResponse(\$params) where \$params is hashref"
        );
        return;
    }
    ...
    $self->addResultData(
        { message => $params->{message}, metadataID => $md_id } );
    return $self;
}
...
sub MetadataKeyRequest {
    ...
    $self->get_LOGGER->error(
        "MetadataKeyRequest  handler  Not   implemented by the service ");
...
sub SetupDataRequest {
    ...
    $self->get_LOGGER->error(
        " SetupDataRequest handler  Not   implemented by the service ");
...
sub MeasurementArchiveStoreRequest {
    ...
    my $response = shift;
    $self->get_LOGGER->error(
" MeasurementArchiveStoreRequest   handler Not   implemented by the service "
    );
    return
" MeasurementArchiveStoreRequest   handler Not   implemented by the service ";
...
#$self->get_LOGGER->debug("  Quering...  ");
    ...
    #$self->get_LOGGER->info("------->".Dumper($element));
    $element->querySQL($queryhash);
    $self->get_LOGGER->debug("  Done  ");

    foreach my $table ( keys %{$queryhash} ) {
        foreach my $entry ( keys %{ $queryhash->{$table} } ) {
            push @{ $queryhash->{"query_$table"} },
              ( $entry => { $oper => $queryhash->{$table}{$entry} } )
              if $queryhash->{$table}{$entry}
                  && !ref( $queryhash->{$table}{$entry} );
        }
    }
    return $queryhash;
}
...
sub processTime {
    ...
    unless (
        ...
        && (
            ...
            || (   $params->{element}
                && blessed $params->{element}
                && $params->{element}->can("querySQL") )
        )
      )
    {
    ...
    $params->{element}->querySQL( $params->{timehash} )
      unless $params->{timehash};

    ##$self->get_LOGGER->debug("  -------> Timestamp=  ",  sub{Dumper($params->{timehash})});

    my %timequery = ( gt => time() - $One_DAY_inSec, lt => time() );

    $timequery{gt} = $params->{timehash}->{'start'}
      if $params->{timehash}->{'start'}
          && !ref( $params->{timehash}->{'start'} );
    if ( $params->{timehash}->{'duration'}
        && !ref( $params->{timehash}->{'duration'} ) )
    {
        $timequery{lt} = $timequery{gt} + $params->{timehash}->{'duration'};
    }
    elsif ( $params->{timehash}->{'end'}
        && !ref( $params->{timehash}->{'end'} ) )
    {
    ...
    unless ( $timequery{eq} || $timequery{gt} ) {
        $self->get_LOGGER->error(
" Failed to get time values,possible missed start time or timestamp from the data element commonTime  "
        );
        return;
    }
    ...
    if ( $params->{timehash}{'cf'} && $params->{timehash}{'resolution'} ) {
        ...
        $timequery{function} =
          $params->{timehash}->{'cf'} eq 'AVERAGE'
          ? 'avg'
          : $params->{timehash}->{'cf'};
    }
    else {
        $self->get_LOGGER->debug( "  ---No cf -  full search : ",
            sub { Dumper($params) } );
    }
    $self->get_LOGGER->debug( "  -------> Processed Timestamp=  ",
        sub { Dumper( \%timequery ) } );
    return \%timequery;
}
...
$Id: Message.pm 2760 2009-04-14 22:32:34Z maxim $
