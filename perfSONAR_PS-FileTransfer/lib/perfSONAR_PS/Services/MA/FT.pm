package perfSONAR_PS::Services::MA::FT;
...
use constant CLASSPATH => 'perfSONAR_PS::Services::MA::FT';
...
our $processName = 'perfSONAR-PS FT MA';
our $nextDataSetID;

=head2 new

create a new instance of the FT MA
...
sub init {
    ...
    eval {
        ...
        # ls stuff
        $self->configureConf( 'enable_registration', undef,
            $self->getConf('enable_registration') );

        # info about service
        $self->configureConf( 'service_name', $processName,
            $self->getConf('service_name') );
        $self->configureConf( 'service_type', 'MA',
            $self->getConf('service_type') );
        ...
        $self->configureConf( 'service_description', $default_description,
            $self->getConf('service_description') );

        my $default_accesspoint;
        if ( $self->getConf("external_address") ) {
            $default_accesspoint =
                'http://'
              . $self->getConf("external_address") . ':'
              . $self->{PORT}
              . $self->{ENDPOINT};
        }
        $self->configureConf( 'service_accesspoint', $default_accesspoint,
            $self->getConf('service_accesspoint') );
        if ( $self->getConf("enable_registration")
            and not $self->getConf("service_accesspoint") )
        {
            $logger->logdie(
"Must have either a service_accesspoint or an external address specified if you enable registration"
            );
        }
        ...
        $self->configureConf( 'db_name', undef,    $self->getConf('db_name') );

        $self->configureConf( 'db_username', undef,
            $self->getConf('db_username') );
        $self->configureConf( 'db_password', undef,
            $self->getConf('db_password') );

        # other
        $self->configureConf(
            'root_hints_url',
            'http://www.perfsonar.net/gls.root.hints',
            $self->getConf('root_hints_url')
        );
        $self->configureConf( 'query_size_limit', undef,
            $self->getConf('query_size_limit') );
    ...
    $logger->info("Initialising FT MA");

    if ($handler) {
        $logger->debug("Setting up message handlers");
        $handler->registerEventHandler( "SetupDataRequest",
            $self->{eventTypes}->tools->filetransfer, $self );
        $handler->registerEventHandler( "MetadataKeyRequest",
            $self->{eventTypes}->tools->filetransfer, $self );
        $handler->registerEventHandler( "SetupDataRequest",
            $self->{eventTypes}->ops->select, $self );
        $handler->registerEventHandler( "MetadataKeyRequest",
            $self->{eventTypes}->ops->select, $self );

        my @eventTypes = (
            $self->{eventTypes}->tools->filetransfer,
            $self->{eventTypes}->ops->select
        );
        $handler->registerMergeHandler( "MetadataKeyRequest", \@eventTypes,
            $self );
        $handler->registerMergeHandler( "SetupDataRequest", \@eventTypes,
            $self );
    ...
    if ( $self->getConf("db_type") eq "SQLite" || "mysql" ) {
        ...
        # setup DB  object
        eval {
            my $dbo = perfSONAR_PS::DB::SQL::FT->new(
                {
...
accessor/mutator for nextDataSetID
...
sub registerLS($) {
    ...
    $logger->info("Registering FT MA with LS");
    my @ls_array = ();
    ...
    if ( !defined $self->ls() ) {
        ...
        #$logger->info("creating new Remote object");
        my $ls = new perfSONAR_PS::Client::LS::Remote( \@ls_array, $ls_conf,
            \@hints_array );

        #$logger->info("new Remote object created");
        $self->ls($ls);
    ...
    my $FTMA = perfSONAR_PS::Datatypes::FT->new();

    #$logger->info("new FT MA created");
    foreach my $metaid ( sort keys %{$metas} ) {
        my $md = $FTMA->ressurectMd( { md_row => $metas->{$metaid} } );
        $md->set_eventType( $self->{eventTypes}->tools->filetransfer );

        #$logger->info("setting ".$self->{eventTypes}.", event Types");
        push @sendToLS, $md->getDOM()->toString();

     #$logger->info("pushed in to sendToLS value = ".$md->getDOM()->toString());

    }

  #$logger->info("Database process done, now register Static is called");
...
sub handleEvent() {
    ...
    my $response = $self->__handleEvent(
        $parameters->{"messageType"},      $parameters->{"rawRequest"},
        \@{ $parameters->{"subject"} },    $parameters->{"data"},
        $parameters->{"filterChain"}->[0], $parameters->{"messageParameters"}
    );

    ##### $response is
    foreach
      my $element ( @{ $response->get_metadata }, @{ $response->get_data } )
    {
...
sub __handleEvent {
    ...
    my ( $self, $messageType, $raw_request, $mds, $data, $filters,
        $message_parameters )
      = @_;
...
#$logger->debug( "Type=$messageType\nmd=".$mds->[0]->toString."\nData=".$data->toString );
    ...
    if ( $messageType =~ m/SetupDataRequest/i ) {
        my $lastID = datasetID();
        datasetID( ++$lastID );
        $logger->info(
            "Found SetupDataRequest Incrementing datasetID for response "
              . $lastID );
    }
    my $arr_filters = [];
    ...
    $logger->info("Unmarshalling into FT object");

    #$logger->info("md = ".Dumper($mds));
    #$logger->info("data = ".Dumper($data));

    my $FTRequest = perfSONAR_PS::Datatypes::FT->new(
        {
    ...
    $FTRequest->registerNamespaces();
    ###$logger->debug("Done...");

    ### pass db handler down request object
    $logger->info("Creating FT response");
    my $FTResponse = perfSONAR_PS::Datatypes::Message->new(
        { type => $responseType, id => $messageIdReturn },
        datasetID() );    # response message
                          # $logger->info( "Done...".Dumper($FTResponse));

    #	foreach my $field ($FTResponse->show_fields('Public')) {
    #		$logger->debug("Pinger Response:  $field= " . $FTResponse->{$field});
    #	}

    #### map namespaces on response
    $logger->debug(" Mapping namespaces on response");
    $FTResponse->set_nsmap( $FTRequest->get_nsmap );
    $logger->debug("Done...");
    ###

    my $evt = $FTRequest->eventTypes;
    ## setting up db object
    $FTRequest->DBO( $self->database );
    my $errorMessage =
      $FTRequest->handle( $type, $FTResponse, $self->{'CONF'}->{'ftma'} );

    $logger->info( "FT RESPONSE: $errorMessage\n" . $FTResponse->asString() );
...
=head1 SYNOPSIS
    ...
    use perfSONAR_PS::Services::MA::FT;
...
$Id: PingER.pm 2772 2009-04-16 19:54:39Z aaron $
