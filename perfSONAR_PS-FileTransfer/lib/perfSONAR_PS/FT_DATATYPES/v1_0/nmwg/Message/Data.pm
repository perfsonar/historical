package  perfSONAR_PS::FT_DATATYPES::v1_0::nmwg::Message::Data;
...
perfSONAR_PS::FT_DATATYPES::v1_0::nmwg::Message::Data  -  this is data binding class for  'data'  element from the XML schema namespace nmwg
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::FT_DATATYPES::v1_0::nmwg::Message::Data->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::FT_DATATYPES::v1_0::nmwg::Message::Data->new({xml => $xml_string});
...
use perfSONAR_PS::FT_DATATYPES::v1_0::nmwgr::Message::Data::Datum;
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::FT_DATATYPES::v1_0::nmwg::Message::Data';
...
sub fromDOM {
	...
	my $element;
        ...
        if ($tagname eq  'commonTime' && $nsid eq 'nmwg' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::FT_DATATYPES::v1_0::nmwg::Message::Data::CommonTime->new($childnode)
                };
               ...
               ($self->get_commonTime && ref($self->get_commonTime) eq 'ARRAY')?push @{$self->get_commonTime}, $element:
                ...
                eval {
                    $element = perfSONAR_PS::FT_DATATYPES::v1_0::nmwgr::Message::Data::Datum->new($childnode)
                };
                if($EVAL_ERROR || !($element  && blessed $element)) {
        ...
        elsif (!($self->get_commonTime) && $tagname eq  'datum' && $nsid eq 'filetransfer' && $self->can("get_$tagname")) {
                eval {
                    $element = perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::Datum->new($childnode)
                };
                if($EVAL_ERROR || !($element  && blessed $element)) {
                    $self->get_LOGGER->logdie(" Failed to load and add  Datum : " . $dom->toString . " error: " . $EVAL_ERROR);
                     return;
               ...
               ($self->get_datum && ref($self->get_datum) eq 'ARRAY')?push @{$self->get_datum}, $element:
                ...
                eval {
                    $element = perfSONAR_PS::FT_DATATYPES::v1_0::nmwg::Message::Data::Key->new($childnode)
                };
                if($EVAL_ERROR || !($element  && blessed $element)) {
