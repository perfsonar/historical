package  perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum;
...
perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum  -  this is data binding class for  'datum'  element from the XML schema namespace filetransfer

=head1 DESCRIPTION

Object representation of the datum element of the filetransfer XML namespace.
Object fields are:
...
=head1 SYNOPSIS
          ...
          use perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum;
          use Log::Log4perl qw(:easy);

          Log::Log4perl->easy_init();

          my $el =  perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum->new({xml => $xml_string});
...
use fields qw(nsmap idmap LOGGER timeType numBytes value valueUnits timeValue  );
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum';
...
sub new {
    ...
    $self->get_nsmap->mapname($LOCALNAME, 'filetransfer');
...
sub  querySQL {
    my ($self, $query) = @_;
     ...
     my %defined_table = ( 'Data' => [   'ID',    'NBytes',    'TransferTime',    'Duration',  ],  );
     $query->{Data}{ID}= [ 'perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum' ] if!(defined $query->{Data}{ID}) || ref($query->{Data}{ID});
     $query->{Data}{NBytes}= [ 'perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum' ] if!(defined $query->{Data}{NBytes}) || ref($query->{Data}{NBytes});
     $query->{Data}{TransferTime}= [ 'perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum' ] if!(defined $query->{Data}{TransferTime}) || ref($query->{Data}{TransferTime});
     $query->{Data}{Duration}= [ 'perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum' ] if!(defined $query->{Data}{Duration}) || ref($query->{Data}{Duration});
            ...
            foreach my $entry (@{$defined_table{$table}}) {
                if(ref($query->{$table}{$entry}) eq 'ARRAY') {
                    foreach my $classes (@{$query->{$table}{$entry}}) {
                         if($classes && $classes eq 'perfSONAR_PS::FT_DATATYPES::v1_0::filetransfer::Message::Data::CommonTime::Datum') {
        
                            if    ($self->get_value && ( (  ($entry eq 'ID') or  ($entry eq 'NBytes') or  ($entry eq 'TransferTime') or  ($entry eq 'Duration')) )) {
