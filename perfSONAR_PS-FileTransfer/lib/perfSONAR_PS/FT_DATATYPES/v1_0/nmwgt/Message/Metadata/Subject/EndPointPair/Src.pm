package  perfSONAR_PS::FT_DATATYPES::v1_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src;
...
perfSONAR_PS::FT_DATATYPES::v1_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src  -  this is data binding class for  'src'  element from the XML schema namespace nmwgt
...
=head1 SYNOPSIS
          ...
          my $el =  perfSONAR_PS::FT_DATATYPES::v1_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src->new($DOM_Obj);

          my $xml_string = $el->asString();

          my $el2 = perfSONAR_PS::FT_DATATYPES::v1_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src->new({xml => $xml_string});
...
Readonly::Scalar our $CLASSPATH =>  'perfSONAR_PS::FT_DATATYPES::v1_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src';
...
sub  querySQL {
    my ($self, $query) = @_;
     ...
     $query->{EndPoints}{SrcIP}= [ 'perfSONAR_PS::FT_DATATYPES::v1_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ] if!(defined $query->{EndPoints}{SrcIP}) || ref($query->{EndPoints}{SrcIP});
     $query->{EndPoints}{SrcIP}= [ 'perfSONAR_PS::FT_DATATYPES::v1_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src' ] if!(defined $query->{EndPoints}{SrcIP}) || ref($query->{EndPoints}{SrcIP});
            ...
            foreach my $entry (@{$defined_table{$table}}) {
                if(ref($query->{$table}{$entry}) eq 'ARRAY') {
                    foreach my $classes (@{$query->{$table}{$entry}}) {
                         if($classes && $classes eq 'perfSONAR_PS::FT_DATATYPES::v1_0::nmwgt::Message::Metadata::Subject::EndPointPair::Src') {
        
                            if    ($self->get_value && ( (  ( ($self->get_type eq 'ipv4')  && $entry eq 'SrcIP')) )) {
                                $query->{$table}{$entry} =  $self->get_value;
                                $self->get_LOGGER->debug(" Got value for SQL query $table.$entry: " . $self->get_value);
                                last;  
                            }

                            elsif ($self->get_text && ( (  ( ($self->get_type eq 'ipv4')  && $entry eq 'SrcIP')) )) {
