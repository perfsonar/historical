package perfSONAR_PS::Client::FT;
...
perfSONAR_PS::Client::FT
=head1 DESCRIPTION

Client API for calling FT MA from a client or another service.  Module
inherits from perfSONAR_PS::FT::MA and overloads callMA, metadataKeyRequest
...
sub callMA {
    ...
    my $msg;
    eval {
        $msg = $self->SUPER::callMA(
            message => $message_dom->asString,
            timeout => 3600
        );
    };
    if ( $EVAL_ERROR || !$msg ) {
        $self->{LOGGER}
          ->error("Message element not found in return. $EVAL_ERROR");
        return;
    }

    #$self->{LOGGER}->error("call done".$msg);
...
Perform a MetadataKeyRequest, the result returned as message DOM:
 ...
 parameters is hashref with filetransfer parameters from this list:  stripes, buffer,block,program,user
...
sub metadataKeyRequest {
    ...
    my $message   = Message->new(
        { 'type' => 'MetadataKeyRequest', 'id' => 'message.' . genuid() } );
    $self->{LOGGER}->debug( "Message: " . $message->asString );
    ...
    if ( $parameters->{xml} ) {
        ...
        $parameters->{eventType} = $eventType->tools->filetransfer;
        my $metadata = $self->getMetaSubj($parameters);

        # create the  element
        my $data = Data->new(
            { 'metadataIdRef' => "metaid$metaid", 'id' => "data$metaid" } );
...
Returns metadata object with filetransfer subj and filetransfer parameters 
   
Manditory:
   
   id => id of the metadata,
   eventType => filetransfer eventtype
...
Optional:
   ...
   src_name =>  source hostname
   dst_name => destination hostname
   parameters => filetransfer parameters from this list:  stripes,buffer,block,program,user
...
sub getMetaSubj {
    ...
    if ( $parameters->{metadata} ) {
        ...
        $md = Metadata->new(
            { key => MetaKey->new( { id => $parameters->{key} } ) } );
        ...
        if ( $parameters->{subject} ) {
            ...
            if ( $parameters->{src_name} || $parameters->{dst_name} ) {
                ...
                $endpoint->set_src(
                    Src->new(
                        { value => $parameters->{src_name}, type => 'ipv4' }
                    )
                ) if $parameters->{src_name};
                $endpoint->set_dst(
                    Dst->new(
                        { value => $parameters->{dst_name}, type => 'ipv4' }
                    )
                ) if $parameters->{dst_name};
        ...
        #while(my($k,$v) =each %{$parameters->{parameters}})
        #{
        #	print ("$k,$v\n");
        #}
        if ( $parameters->{parameters}
            && ref $parameters->{parameters} eq 'HASH' )
        {
            my @params;
            my $meta_params = FtpParams->new( { id => "params$metaid" } );
            foreach my $p (
                qw/stripes blocksize buffersize streams srcpath destpath program user/
              )
            {
    ...
    $md->set_metadataIdRef( "metaid" . $parameters->{idRef} )
      if $parameters->{idRef};
...
sub getMetaTime {
    ...
    if ( $parameters->{metadata} ) {
        ...
        my @params;
        my $time_params = FtpParams->new( { id => "params$metaid" } );
        if ( $parameters->{start} ) {
            push @params,
              Parameter->new(
                {
...
sub setupDataRequest {
    ...
    my $metaid  = genuid();
    my $message = Message->new(
        { 'type' => 'SetupDataRequest', 'id' => 'message.' . genuid() } );
...
sub getPair {
    ...
    $parameters->{id} = genuid();
    my $md_filetransfer = $self->getMetaSubj($parameters);

    # create the  element
    my $data = Data->new(
        {
            'metadataIdRef' => "metaid$parameters->{id}",
            'id'            => "data$parameters->{id}"
        }
    );
    return unless $message;
    $message->addMetadata($md_filetransfer) if $md_filetransfer;
...
sub getMetaData {
    ...
    unless ( $response && $response->isa('perfSONAR_PS::Datatypes::Message') ) {
        $self->{LOGGER}
          ->error(" Attempted to get metadata from empty response");
        return;
    }
    foreach my $md ( @{ $response->get_metadata } ) {
        unless ( $md->get_key && $md->get_subject ) {
            $self->{LOGGER}
              ->info("Skipping metadata - key or subject is missing ");
            next;
        }

        #$self->{LOGGER}->info("key or subject found");
        my $key_id  = $md->get_key->get_id;
        my $subject = $md->get_subject;       #first subj
        unless ( $subject && $subject->get_endPointPair ) {
            $self->{LOGGER}->get_error(
                "Malformed metadata in response -  subject is missing ");
            next;
        }
        ...
        #$self->{LOGGER}->info("md = ".Dumper($md));
        foreach my $params ( @{ $md->get_parameters } ) {
            foreach my $param ( @{ $params->get_parameter } ) {
                if ( $param->get_name =~
m/stripes|buffersize|blocksize|streams|srcpath|destpath|user|program/i
                  )
                {
                    $parameters->{ $param->get_name } =
                      $param->get_value ? $param->get_value : $param->get_text;
...
ref to hash with datums ( name => value )
     ...
     "$src:$dst:$packetSize" => { 
	  ...
	  keys => [ array of metadata keys assigned with "$src:$dst" ],
          metaIDs	 => [ array of metadata ids] ]
     }
...
sub getData {
    ...
    foreach my $uniq_key ( keys %{$metadata} ) {
        ...
        for ( my $count = 0 ; $count < scalar( @{$metaids} ) ; $count++ ) {
            ...
            my $parameters = $metadata->{$uniq_key}->{parameters}->[$count];
            ...
            foreach my $ctime ( @{$times} ) {
                ...
                next
                  unless $ctime->get_datum && ref $ctime->get_datum eq 'ARRAY';
                foreach my $datum ( @{ $ctime->get_datum } ) {
                    while ( my ( $k, $v ) = each %{$parameters} ) {
                        $data->{$key}{$timev}{$k} = $v;
                    }
                    $data->{$key}{$timev}{throughput} = $datum->get_value;
                    if ( $datum->get_value > 0 ) {
                        $data->{$key}{$timev}{units}  = $datum->get_valueUnits;
                        $data->{$key}{$timev}{nbytes} = $datum->get_numBytes;
                    }
                }
...
=head1 SYNOPSIS
    ...
    my $metadata = qq{ <nmwg:metadata id="metaBase">
        <filetransfer:subject xmlns:filetransfer="http://ggf.org/ns/nmwg/tools/filetransfer/1.0/" id="subject1">
         <nmwgt:endPointPair xmlns:nmwgt="http://ggf.org/ns/nmwg/topology/2.0/">
            <nmwgt:src type="ipv4" value="hostname/IP address for source"/> 
             <nmwgt:dst type="ipv4" value="IP adress of the destination"/> 
        </nmwgt:endPointPair>
       </pinger:subject>
       <nmwg:eventType>http://ggf.org/ns/nmwg/tools/filetransfer/1.0/</nmwg:eventType>
       </nmwg:metadata>
  };
   

    my $ma = new perfSONAR_PS::Client::FT(
      { instance => "http://localhost:9000/perfSONAR_PS/services/FT/ma"}
    );
	...
	dst_name => 'some.lab.gov',
      ...
      foreach my $src_dst (keys %{$self->getMetaData($result)})
           print "Src:DST key = $src_dst_packetsize \n";
      }
