...
my %ns = (
    ...
    filetransfer => "http://ggf.org/ns/nmwg/tools/filetransfer/1.0/",
...
"http://ggf.org/ns/nmwg/tools/org/perfsonar/service/lookup/summarization/2.0/",
    ...
    ctrlplane_oct =>
      "http://ogf.org/schema/network/topology/ctrlPlane/20071023/",
...
if ( not $status or $HELP ) {
    ...
    print
"\t$0 [--verbose --help --config=config.file --piddir=/path/to/pid/dir --pidfile=filename.pid --logger=logger/filename.conf --user=[user to run as] --group=[group to run as] --output=/path/to/log/output --noclean\n";
    exit(1);
}
...
if ( $RUNAS_USER and $RUNAS_GROUP ) {
    ...
    print
"Error: You need to specify both the user and group if you specify either\n";
    exit(-1);
}
...
unless ( exists $conf{"max_worker_processes"}
    and $conf{"max_worker_processes"} )
{
    $logger->warn("Setting maximum worker processes at 32");
    $conf{"max_worker_processes"} = 32;
}

unless ( exists $conf{"ls_registration_interval"}
    and $conf{"ls_registration_interval"} )
{
...
unless ( exists $conf{"disable_echo"} and $conf{"disable_echo"} ) {
    $logger->warn(
        "Enabling echo service for each endpoint unless specified otherwise");
    $conf{"disable_echo"} = 0;
}
...
foreach my $port ( keys %{ $conf{"port"} } ) {
    ...
    unless ( exists $conf{"port"}->{$port}->{"endpoint"}
        and $conf{"port"}->{$port}->{"endpoint"} )
    {
    ...
    foreach my $key ( keys %{ $conf{"port"}->{$port}->{"endpoint"} } ) {
        ...
        my %endpoint_conf = %{
            mergeConfig( \%port_conf,
                $conf{"port"}->{$port}->{"endpoint"}->{$key} )
          };

        $service_configs{$port}->{"endpoint"}->{$fixed_endpoint} =
          \%endpoint_conf;

        next
          if exists $endpoint_conf{"disabled"}
              and $endpoint_conf{"disabled"} == 1;

        $logger->debug("Adding endpoint $fixed_endpoint to $port");

        $handlers{$port}->{$fixed_endpoint} =
          perfSONAR_PS::RequestHandler->new();
        ...
        if ( ref $endpoint_conf{"module"} eq "ARRAY" ) {
            ...
            $logger->debug(
                "Modules is not an array: " . ref( $endpoint_conf{"module"} ) );
        ...
        foreach my $module (@endpoint_modules) {
            unless ( exists $modules_loaded{$module}
                and $modules_loaded{$module} )
            {
                load $module;
...
foreach my $port ( keys %listeners ) {
    ...
    if ( $pid == 0 ) {
        ...
        psService( $listeners{$port}, $handlers{$port},
            $service_configs{$port} );
        exit(0);
    }
    elsif ( $pid < 0 ) {
...
sub psService {
    ...
    while (1) {
        if ( $max_worker_processes > 0 ) {
            while ( %child_pids
                and scalar( keys %child_pids ) >= $max_worker_processes )
            {
        ...
        if (%child_pids) {
            ...
            $logger->debug( "Reaping children (total: "
                  . scalar( keys %child_pids )
                  . ") at time "
                  . $time );
          ...
          # reap any children that have finished or outlived their allotted time
            foreach my $pid ( keys %child_pids ) {
                if ( waitpid( $pid, WNOHANG ) ) {
                    ...
                    and $child_pids{$pid}->{"child_timeout_length"} > 0 )
                {
                    $logger->error("Pid $pid timed out.");
                    kill 9, $pid;

                    my $msg =
                        "Timeout occurred, current limit is "
                      . $child_pids{$pid}->{"child_timeout_length"}
                      . " seconds. Try decreasing the breadth of your search if possible.";
                    my $resMsg = getErrorResponseMessage(
                        eventType   => "error.common.timeout",
                        description => $msg
                    );
        ...
        if ( not defined $handle ) {
            my $msg =
"Accept returned nothing, likely a timeout occurred or a child exited";
            $logger->debug($msg);
        }
        else {
            $logger->info(
                "Received incoming connection from:\t" . $handle->peerhost() );
            my $pid = fork();
            if ( $pid == 0 ) {
                %child_pids = ();
                ...
                unless ($http_request) {
                    my $msg = "No HTTP Request received from host:\t"
                      . $handle->peerhost();
                    $logger->error($msg);
                    $handle->close;
                    exit(-1);
  ...
  #$logger->info("handle = ".Dumper($handle)."\nhttp_request = ".$http_request);
                my $request =
                  perfSONAR_PS::Request->new( $handle, $http_request );
                if ( not exists $handlers->{ $request->getEndpoint() } ) {
                    my $msg = "Received message with has invalid endpoint: "
                      . $request->getEndpoint();
                    $request->setResponse(
                        getErrorResponseMessage(
                            eventType   => "error.common.transport",
                            description => $msg
                        )
                    );
                    $request->finish();
                ...
                my $max_worker_lifetime =
                  $service_config->{"max_worker_lifetime"};
                my %child_info = ();
...
sub registerLS {
    ...
    my $sleep_time = q{};
    if ( exists $args->{"conf"}->{"ls_registration_interval"}
        and $args->{"conf"}->{"ls_registration_interval"} )
    {
        $sleep_time = $args->{"conf"}->{"ls_registration_interval"};
    }
    elsif ( exists $args->{"conf"}->{"ls"}->{"ls_registration_interval"}
        and $args->{"conf"}->{"ls"}->{"ls_registration_interval"} )
    {
        $sleep_time = $args->{"conf"}->{"ls"}->{"ls_registration_interval"};
    }
    elsif ( exists $args->{"conf"}->{"gls"}->{"ls_registration_interval"}
        and $args->{"conf"}->{"gls"}->{"ls_registration_interval"} )
    {
    ...
    while (1) {
        $sleep_time = $args->{"conf"}->{"ls_registration_interval"}
          unless $sleep_time;
        $logger->info(" calling register LS");
        my $ServiceCheck = $service->registerLS( \$sleep_time );
        $logger->info(
            "register LS called, service result = " . $ServiceCheck );
...
sub maintenance {
    ...
    while (1) {
        ...
        eval {
            $cleanStatus =
              $service->cleanLS( { error => \$error, noclean => $CLEANFLAG } )
              if $service->can("cleanLS");
            $sumStatus = $service->summarizeLS( { error => \$error } )
              if $service->can("summarizeLS");
        };
        if ( my $e = catch std::exception ) {
            $logger->error(
                "Problem running service maintenance: " . $e->what() );
        }
        elsif ($EVAL_ERROR) {
            $logger->error(
                "Problem running service maintenance: " . $EVAL_ERROR );
        }
...
sub handleRequest {
    ...
    #$logger->info("handleRequest=".Dumper($request));
    my $handleRequestStartTime = time();
    my $messageId              = q{};
    try {
        my $error = q{};
        if ( $request->getRawRequest->method ne "POST" ) {
            my $msg =
"Received message with an invalid HTTP request, are you using a web browser?";
            $logger->error($msg);
            throw perfSONAR_PS::Error_compat( "error.common.transport", $msg );
        }

        $request->parse( \%ns, \$error );
        throw perfSONAR_PS::Error_compat( "error.transport.parse_error",
            "Error parsing request: $error" )
          if $error;

        #$logger->debug("request DOM = ".Dumper($request->getRequestDOM()));
    ...
    otherwise {
        ...
        $request->setResponse(
            getErrorResponseMessage(
                messageIdRef => $messageId,
                eventType    => "error.common.internal_error",
    ...
    my $handleRequestEndTime = time();
    my $handleRequestTotalTime =
      $handleRequestEndTime - $handleRequestStartTime;
    $logger->info(
        "Total time to handle request = " . $handleRequestTotalTime . " secs" );
    return;
}
...
sub lockPIDFile {
    ...
    sysopen( PIDFILE, $pidfile, O_RDWR | O_CREAT )
      or croak("Couldn't open pidfile");
...
$Id: perfsonar-daemon.pl 2666 2009-03-24 14:17:20Z aaron $
